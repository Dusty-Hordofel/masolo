...
export const useFileUploadToCloudinary = (
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    console.log("🚀 ~ uploadFiles ~ files:FILES", files)
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (data.secure_url && data.public_id && data.original_filename) {
          ...
          console.log("🚀 ~ uploadFiles ~ results:IMAGES", results)
  ...
  const uploadFile = async (file: File) => {
    console.log("🚀 ~ uploadFile ~ file:FILES", file)
     ...
     if (!file) {
      ...
      console.log("🚀 ~ uploadFile ~ formData:", UPLOAD_CONFIG.URL)
     /*  console.log("🚀 ~ uploadFile ~ formData:", formData) */
      // Download to Cloudinary
      const response = await fetch(UPLOAD_CONFIG.URL, {
        method: "POST",
      ...
      const cloudinaryData = await response.json();
      console.log("🚀 ~ uploadFile ~ cloudinaryData:MMN", cloudinaryData)
